...
void calcDewPoint(){
  ...
  SUM += log10(1013.246);
  double VP = pow(10, SUM-3) * wx.humidite;
...
const char PAGE_infos[] PROGMEM = "<html>\
  ...
  <body>\
    ...
    <fieldset>\
      ...
      <p>Temperature: {{degC}} degC</p>\
      <p>Pressure: {{mBar}} mBar</p>\
      <p>Relative Humidity: {{rHum}}%</p>\
      <p>Dew point: {{DPdegC}} degC </p>\
...
void handleRoot() {
  ...
  page.replace("{{rHum}}", String(wx.humidite,2));
  ...
  page.replace("{{APRS_CMNT}}", APRS_CMNT);
...
void handleSubmit(){
  ...
  if (server.args() > 0 ) {
    ...
    if (server.argName(0) == "SendAPRS" && server.arg(0) == "true"){
      ...
      message += "<p>Sending packet to APRS server....</p>";
      message += "<form><input type='button' value='back' onclick='history.back()'></form>";
      message += FPSTR(HTTP_FOOT);
    ...
    if (server.argName(0) == "SendWUNDER" && server.arg(0) == "true"){
      ...
      message += "<p>Sending packet to Wetaher Underground server...</p>";
      message += "<form><input type='button' value='back' onclick='history.back()'></form>";
      message += FPSTR(HTTP_FOOT);
    ...
    if (server.argName(0) == "NTPSync" && server.arg(0) == "true"){
      ...
      message += "<p>NTP sync request to server...</p>";
      message += "<form><input type='button' value='back' onclick='history.back()'></form>";
      message += FPSTR(HTTP_FOOT);
...
void updateServer()
{
  ...
  if (station.logger == 1) {
    if (SPIFFS.exists("/logger.txt") == 1) {
      ...
      sprintf(buffer, "%02d:%02d:%02d;", dateTime.hour, dateTime.minute, dateTime.second);
      f.print(buffer);
      sprintf(buffer, "%03d;%02d;%05d\n", (int)(wx.temperatureC), (int)(wx.humidite), (int)(wx.pression));
      f.print(buffer);
      f.close();
    }
  }
...
void getBmeValues(){
    ...
    wx.humidite =  mySensor.readFloatHumidity();
